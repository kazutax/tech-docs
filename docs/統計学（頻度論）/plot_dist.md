...
layout: default
title: 有意水準と検出力
description: "適切なサンプルサイズを選ぶこと、α エラー（第1種過誤）と β エラー（第2種過誤）を理解することが、効果的な A/B テスト実施の鍵。それらの理解に必要な「有意水準」と「検出力」とは何か、Python スクリプトと図解を交えて解説する。"
nav_order: 1
parent: 統計学（頻度論）
has_children: false
has_toc: false
...
+ 詳しい計算例は、別ページ [A/Bテストのためのサンプルサイズ計算](/tech-docs/docs/統計学（頻度論）/sample_size/){:target="_blank"} にまとめているので参照。
